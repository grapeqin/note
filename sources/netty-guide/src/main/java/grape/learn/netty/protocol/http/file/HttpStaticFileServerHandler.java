 ...
 * under the License.
 */
package grape.learn.netty.protocol.http.file;
...
public class HttpStaticFileServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
  ...
  protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
    ...
    if (ifModifiedSince != null && !ifModifiedSince.isEmpty()) {
      DateTimeFormatter dateTimeFormatter =
          DateTimeFormatter.ISO_DATE_TIME.withLocale(Locale.CHINA);
      LocalDateTime ifModifiedSinceDate = LocalDateTime.parse(ifModifiedSince, dateTimeFormatter);

      // Only compare up to the second because the datetime format we send to the client
      // does not have milliseconds
      long ifModifiedSinceDateSeconds = ifModifiedSinceDate.getSecond();
  ...
  private static void setDateHeader(FullHttpResponse response) {
    DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ISO_DATE_TIME.withLocale(Locale.CHINA);

    LocalDateTime time = LocalDateTime.now();
    response.headers().set(DATE, dateTimeFormatter.format(time));
  }
  ...
  private static void setDateAndCacheHeaders(HttpResponse response, File fileToCache) {
    ...
    // Add cache headers
    time.plus(HTTP_CACHE_SECONDS, ChronoUnit.SECONDS);
    response.headers().set(EXPIRES, dateTimeFormatter.format(time));
        ...
        .set(
            ...
            dateTimeFormatter.format(
                LocalDateTime.ofInstant(
                    Instant.ofEpochMilli(fileToCache.lastModified()), ZoneId.of("Asia/Shanghai"))));
  }

  /**
   * Sets the content type header 'text/plain' for the HTTP Response
   *
   * @param response HTTP response
